# About Jekyll

GitHub Pages and Jekyll are helpful because they make it easier to build a website without setting up a separate web server. A user can create files, commit them to GitHub, and then publish those files online as webpages.

## More Terms

| Term | Definition |
|---|---|
| Markdown | A simple formatting language used to create headings, paragraphs, links, tables, and images. |

## Question

How would I add custom design or styling to a GitHub Pages website using CSS if possible?

![Jekyll logo](https://jekyllrb.com/img/logo-2x.png)

Return to the homepage: [Home](ind.html)
